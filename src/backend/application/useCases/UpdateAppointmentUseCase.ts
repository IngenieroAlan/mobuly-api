...
export class UpdateAppointmentUseCase {
  ...
  async execute(userId: string, id: string, dto: UpdateAppointmentDTO): Promise<Appointment> {
    ...
    const servicioId = dto.servicioId !== undefined ? dto.servicioId : existing.servicioId;
    const fechaInicio = dto.fechaInicio ? new Date(dto.fechaInicio) : existing.fechaInicio;
    let fechaFin: Date;
    ...
    if (dto.servicioId && dto.fechaInicio) {
      ...
      fechaFin = new Date(fechaInicio.getTime() + service.duracionMinutos * 60_000);
    ...
    const updated = Appointment.create({
      ...
      notasCita: dto.notasCita !== undefined ? dto.notasCita : existing.notasCita,
